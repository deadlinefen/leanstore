...
struct DTRegistry {
   ...
   std::unordered_map<DataStructureId, std::tuple<DTType, void*, string>> dt_instances_ht;
   static DTRegistry global_dt_registry;
   
   void registerDatastructureType(DTType type, DTRegistry::DTMeta dt_meta);
   DataStructureId registerDatastructureInstance(DTType type, void* root_object, string name);
   void registerDatastructureInstance(DTType type, void* root_object, string name, DataStructureId dt_id);
   
   void iterateChildrenSwips(DataStructureId dtid, BufferFrame&, std::function<bool(Swip<BufferFrame>&)>);
   ParentSwipHandler findParent(DataStructureId dtid, BufferFrame&);
   SpaceCheckResult checkSpaceUtilization(DataStructureId dtid, BufferFrame&);
   // Pre: bf is shared/exclusive latched
   void checkpoint(DataStructureId dt_id, BufferFrame& bf, u8*);
   // Recovery / SI
   void undo(DataStructureId dt_id, const u8* wal_entry, u64 tts);
   void todo(DataStructureId dt_id, const u8* entry, const u64 version_worker_id, u64 version_tts, const bool called_before);
   void unlock(DataStructureId dt_id, const u8* entry);
   // Serialization
   std::unordered_map<std::string, std::string> serialize(DataStructureId dt_id);
   void deserialize(DataStructureId dt_id, std::unordered_map<std::string, std::string> map);
};
