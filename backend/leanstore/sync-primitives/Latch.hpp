...
constexpr static uint64_t LATCH_EXCLUSIVE_BIT = 1ull;
constexpr static uint64_t LATCH_VERSION_MASK = ~(0ull);
constexpr size_t kLatchAlignasSize = 64;

using VersionType = std::atomic<uint64_t>;
struct alignas(kLatchAlignasSize) HybridLatch {
  VersionType version;
...
enum class LATCH_FALLBACK_MODE : uint8_t {
  SHARED = 0,
  EXCLUSIVE = 1,
  JUMP = 2,
  SPIN = 3,
...
struct HybridLatchGuard {
  ...
  HybridLatchGuard &operator=(HybridLatchGuard &&other) {
    unlock();
  ...
  inline void toExclusive() {
    ...
    if (state == GUARD_STATE::EXCLUSIVE) return;
    if (state == GUARD_STATE::OPTIMISTIC) {
      const uint64_t new_version = version + LATCH_EXCLUSIVE_BIT;
  ...
  inline void toShared() {
    ...
    if (state == GUARD_STATE::SHARED) return;
    if (state == GUARD_STATE::OPTIMISTIC) {
      latch->mutex.lock_shared();
